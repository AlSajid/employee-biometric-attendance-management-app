...
import { Document, Page, View, Text, PDFDownloadLink, PDFViewer, StyleSheet } from '@react-pdf/renderer';
                ...
                <View style={styles.table}>
                    <View style={styles.tableRow}>
                        {
                            options.map((data, index) => (
                                <View style={{ ...styles.tableColHeader, width: data.width }} key={index}>
                    ...
                    {users?.map((info, i) => (
                        <View style={styles.tableRow} key={i}>
                            {
                                options.map((data, index) => (
                                    <View style={{ ...styles.tableCol, width: data.width }} key={index}>
                    ...
                    <>
                        <PDFDownloadLink document={
                            ...
                            {({ blob, url, loading, error }) =>
                                loading
                                    ? 'Loading document...'
                                    : <button className='w-full h-5/6'>Generate Report</button>
                            }
                        </PDFDownloadLink>
                    </>
                )
